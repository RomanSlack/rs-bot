...
> **The fastener interface is now consistent end to end (2026-08-06).** This
> table had drifted from the CAD - 40 deleted M2 case screws, missing the M3x6
> horn and rod-clamp bolts, and 16 inserts with no bore to seat in - and every
> one is fixed. The count is **8 inserts, not 16**: the 8 "chassis side plates ->
> top" were for a joint a one-piece chassis does not have and came off. All 8
> real insert bores are cut (4 in the chassis Pi standoffs, 4 in the ankle
> yokes), so `cad.fasteners inserts_seated` reads empty. The **material** is
> still the block: PA6-CF strength on a coupon, not the order sheet's job. See
> `road-to-order.md`.
>
> The list below is the true output of `uv run python -m cad.fasteners`
> (`fastener_order()`), and a test holds it there. Regenerate, do not hand-edit.

| part | qty |
|---|---|
| M3 x 8 socket cap | 32 |
| M3 x 6 socket cap (horn centre screw) | 8 |
| M3 x 12 socket cap (rod-2 clamp) | 4 |
| M3 nut, captive (rod-2 clamp) | 4 |
| M2.5 x 8 socket cap | 4 |
| M2.5 x 6 socket cap | 4 |
| M2.5 heat-set insert, 3.5 mm bore x 4.0 deep | 8 |

Buy 25% spares. They cost nothing and one missing screw stops a build for a week.
...
**1.749 kg**, and every line below is `uv run python -m cad.masses`, summed,
rather than a number carried forward from the last time someone wrote this out:

| | g |
|---|---|
| printed structure, all nine parts | 359 |
...
The 1 g back on 2026-08-05 is the seating faces that stop the four floating
servos: an end wall under each hip servo and a datum wall on each roll cradle.

Also on 2026-08-05, 22 g came off on paper without a part changing: the printed
structure had been quoted at PETG's 1.270 g/cm3 - what it was costed in - while
it is ORDERED in PA6-CF at 1.19 (docs/materials.md). The density now lives in
one file, cad/material.py, that the CAD and the sim both import. The balancer
still stands 60 s and rejects a 1.0 N.s shove at the lighter mass.
